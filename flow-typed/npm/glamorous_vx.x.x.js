// flow-typed signature: 387fb4b04c8ee313a5f2986e52c8b129
// flow-typed version: <<STUB>>/glamorous_v^4.12.1/flow_v0.68.0
...
declare module 'glamorous' {
  declare type SingleOrArray<Properties, Key : $Enum<Properties>> = {
    [key: Key]: $ElementType<Properties, Key> | Array<$ElementType<Properties, Key>>
  };
  // See CSS 3 CSS-wide keywords https://www.w3.org/TR/css3-values/#common-keywords
  // See CSS 3 Explicit Defaulting https://www.w3.org/TR/css-cascade-3/#defaulting-keywords
  // "all CSS properties can accept these values"
  declare type CSSWideKeyword = 'initial' | 'inherit' | 'unset';
  ...
  declare interface CSSPropertiesCompleteSingle {
    /**
    ...
    alignmentBaseline?: CSSWideKeyword | 'auto' | 'baseline' | 'before-edge' | 'text-before-edge' |
      'middle' | 'central' | 'after-edge' | 'text-after-edge' | 'ideographic' | 'alphabetic' |
      'hanging' | 'mathematical' | 'use-script';

    /**
     * Defines a length of time to elapse before an animation starts, allowing an animation to begin
      execution some time after it is applied.
     */
     ...
     * the element's background color. The value is a list of blend modes that corresponds to each
     * background image. Each element in the list will apply to the corresponding element
     * of background-image. If a property doesn’t have enough comma-separated values to match
     * the number of layers, the UA must calculate its used value by repeating the list of values
     * until there are enough.
     */
    backgroundBlendMode?: CSSWideKeyword | 'normal' | 'multiply' | 'screen' | 'overlay' | 'darken' |
     ...
     * border in a single declaration. It can be used to set border-width, border-style
     * and border-color, or a subset of these.
     */
     ...
     */
    borderBottomStyle?: CSSWideKeyword | 'none' | 'hidden' | 'dotted' | 'dashed' | 'solid' |
     ...
     */
    display?: 'none' | 'block' | 'inline' | 'run-in' | 'flow' | 'flow-root' | 'table' | 'flex' |
      'grid' | 'ruby' | 'subgrid' | 'block flow' | 'inline table' | 'flex run-in' | 'list-item' |
      'list-item block' | 'list-item inline' | 'list-item flow' | 'list-item flow-root' |
      ...
      'ruby-text' | 'ruby-base-container' | 'ruby-text-container' | 'contents' | 'none' |
    ...
    flexFlow?: CSSWideKeyword;
      ...
      // flex direction + flex wrap | 'row nowrap' | 'row wrap' | 'row wrap-reverse' | 'row-reverse nowrap' | 'row-reverse wrap' | 'row-reverse wrap-reverse' | 'column nowrap' | 'column wrap' | 'column wrap-reverse' | 'column-reverse nowrap' | 'column-reverse wrap' | 'column-reverse wrap-reverse'
      // flex direction + flex wrap (reverse order) | 'nowrap row' | 'wrap row' | 'wrap-reverse row' | 'nowrap row-reverse' | 'wrap row-reverse' | 'wrap-reverse row-reverse' | 'nowrap column' | 'wrap column' | 'wrap-reverse column' | 'nowrap column-reverse' | 'wrap column-reverse' | 'wrap-reverse column-reverse'
  ...
  declare type CSSPropertiesComplete =
    SingleOrArray<CSSPropertiesCompleteSingle, $Enum<CSSPropertiesComplete>>;

  declare interface CSSPropertiesLossy {
    [propertyName: string]: string | number | CSSPropertiesComplete | typeof undefined |
      Array<$ElementType<CSSPropertiesComplete, $Enum<CSSPropertiesComplete>>> |
      CSSPropertiesLossy;
  }

  declare type CSSProperties = CSSPropertiesComplete | CSSPropertiesLossy;

  declare type CSSPropertiesRecursive = CSSProperties | CSSPropertiesArray | CSSFunction

  // TODO: This could be made generic. Issue PR if you're so inclined!
  declare interface CSSFunction {
    (props: {}): CSSPropertiesRecursive
  }

  declare type CSSPropertiesArray = Array<CSSPropertiesRecursive>;
